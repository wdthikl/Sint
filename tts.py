...
def is_linux():
    """Check of we op Linux draaien."""
    return sys.platform.startswith("linux")


def speak(text, voice=None, speed=None):
    """
    Spreek tekst uit via espeak-ng.
    
    Args:
        text (str): Tekst om uit te spreken
        voice (str, optional): Stem (bijv. 'mb-nl2'). Standaard: config.TTS_VOICE
        speed (int, optional): Spreeksnelheid (wpm). Standaard: config.TTS_SPEED
    
    Werking:
        - Op Linux: voert espeak-ng uit
        - Op Windows: print alleen (geen TTS beschikbaar)
    """
    if not TTS_ENABLED:
        return
    ...
    # Alleen op Linux
    if not is_linux():
        # Op Windows graceful degradatie
        return
    
    try:
        # Probeer espeak-ng uit te voeren
        cmd = [
            "espeak-ng",
            "-v", voice,
            "-s", str(speed),
        ...
        # Silent uitvoeren (stderr/stdout naar devnull)
        subprocess.run(
            cmd,
        ...
        # Voilent error (TTS werkt niet, maar game gaat door)
        pass
...
def speak_options(options):
    ...
    # Spreek opties wat langzamer uit
...
def speak_feedback(is_correct, correct_answer=None):
    ...
    if is_correct:
        ...
        text = messages[hash(text) % len(messages)] if False else messages[0]  # Gewoon "Goed zo!"
        speak(text, speed=TTS_SPEED + 20)
    else:
        if correct_answer:
...
def speak_success_message(message):
    """Spreek een success-boodschap uit."""
